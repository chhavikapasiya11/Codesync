...
export default function GitVersionPanel() {
  ...
  const [showCompareUI, setShowCompareUI] = useState(false); 
  ...
  const fetchVersions = async () => {
    ...
    setLoading(true);
    try {
      const res = await API.get(`/versions/${sessionId}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const saveVersion = async () => {
    ...
    try {
      await API.post("/versions/save", formData, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const restoreVersion = async (versionId) => {
    try {
      ...
      await API.post(`/code/${sessionId}/files`, { files: restored });
  ...
  return (
    <div style={{ padding: "16px", paddingTop: "80px", backgroundColor: "#121212", color: "#eee", minHeight: "100vh" }}>
      ...
      <textarea
        value={combinedCode}
        readOnly
        style={{ width: "100%", height: "140px", backgroundColor: "#2d2d2d", color: "#ccc", marginBottom: "8px", padding: "10px", border: "1px solid #444" }}
      />
      ...
      <input
        ...
        style={{ width: "100%", padding: "8px", marginBottom: "10px", borderRadius: "4px", border: "1px solid #555" }}
      />
      ...
      <div style={{ marginBottom: "12px", display: "flex", gap: "10px", flexWrap: "wrap" }}>
        <button
          type="button"
          onClick={saveVersion}
          style={{ padding: "8px 12px", backgroundColor: "#007acc", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer" }}
        >
          💾 Save Version
        </button>
        ...
        <button
          ...
          style={{ padding: "8px 12px", backgroundColor: "#2196f3", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }}
        >
          🔍 Compare Versions
        </button>
      </div>
      ...
      {showCompareUI && (
        <div style={{ backgroundColor: "#1e1e1e", padding: "16px", borderRadius: "8px", border: "1px solid #444", marginBottom: "16px" }}>
          <h3 style={{ color: "#00e676", marginBottom: "10px" }}>Select exactly 2 versions to compare:</h3>
          <p style={{ fontSize: "14px", marginBottom: "8px", color: "#bbb" }}>Tick 2 checkboxes from the list below, then click Confirm.</p>
          <div>
            <button
              onClick={() => {
            ...
            <button
              ...
              style={{ marginLeft: "10px", padding: "10px 16px", backgroundColor: "#555", color: "#fff", borderRadius: "6px", border: "none", cursor: "pointer" }}
            >
              ❌ Cancel
            </button>
          </div>
      ...
      {loading ? (
        ...
        <ul style={{ listStyle: "none", padding: 0 }}>
          ...
          {versions.map((v) => (
            <li key={v._id} style={{ marginBottom: "10px", padding: "10px", backgroundColor: "#2d2d2d", borderRadius: "4px" }}>
              <label>
                <input
                  type="checkbox"
                ...
                <button
                  ...
                  style={{ marginLeft: "10px", padding: "4px 8px", backgroundColor: "#ff9800", color: "#000", border: "none", borderRadius: "4px", cursor: "pointer" }}
                >
                  ♻ Restore
                </button>
              </label>
      ...
      {restoredFiles.length > 0 && (
        <div style={{ marginTop: "30px" }}>
          ...
          {restoredFiles.map((file, idx) => (
            <div key={idx} style={{ background: "#1e1e1e", padding: "12px", marginBottom: "16px", borderRadius: "6px", border: "1px solid #444" }}>
              <h4 style={{ color: "#00e676" }}>{file.name}</h4>
              <pre style={{ whiteSpace: "pre-wrap", fontSize: "14px", backgroundColor: "#2d2d2d", padding: "10px", borderRadius: "4px" }}>
                {file.content}
              </pre>
            </div>
          ))}
            ...
            <div key={idx} style={{ background: "#1e1e1e", padding: "12px", marginBottom: "16px", borderRadius: "6px", border: "1px solid #444" }}>
              ...
              <pre style={{ whiteSpace: "pre-wrap", fontSize: "14px" }}>
                {fileDiff.diff.map((part, i) => {
                  const bg = part.added ? "rgba(0,255,0,0.15)" : part.removed ? "rgba(231, 20, 20, 0.15)" : "";
                  return (
